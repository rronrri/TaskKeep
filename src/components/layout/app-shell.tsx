...
export async function AppShell({ role, children }: { role: UserRole; children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen md:grid md:grid-cols-[250px_1fr]">
      <aside className="border-b border-[var(--line)] bg-[var(--paper-deep)] p-5 md:min-h-screen md:border-b-0 md:border-r">
        <div className="mb-8 font-display text-2xl font-bold text-[var(--primary)]">TaskKeep</div>
        <nav aria-label="Navegación principal" className="flex gap-2 overflow-auto md:flex-col">
          {links.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href} className="flex shrink-0 items-center gap-3 rounded-md border-l-[3px] border-transparent px-3 py-2.5 text-sm font-semibold text-[var(--ink-soft)] hover:border-[var(--primary)] hover:bg-[var(--primary-wash)] hover:text-[var(--primary)]">
              <Icon size={19} />{label}
            </Link>
          ))}
        </nav>
      ...
      <div>
        <header className="flex items-center justify-between border-b border-[var(--line)] bg-[var(--surface)] px-5 py-4 md:px-8">
          <div><p className="font-display text-sm font-bold text-[var(--primary)]">TaskKeep</p><p className="folio">Gestión de tareas</p></div>
