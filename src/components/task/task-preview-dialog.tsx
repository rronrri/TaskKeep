...
const requestLabels = { pending_review: "Pendiente de revisiÃ³n", approved: "Aprobada", rejected: "Rechazada" };
...
export function TaskPreviewDialog({
  ...
  return (
    <AppDialog open onOpenChange={onOpenChange} title={task.title} description="Detalle, conversaciÃ³n y actividad de la tarea." size="lg">
      <div className={`rounded-2xl border p-5 ${priority.card}`}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <span className={`rounded-full px-3 py-1 text-xs font-extrabold ${priority.badge}`}>{priority.label}</span>
          {task.is_pinned && <span className="flex items-center gap-1 text-xs font-bold"><Pin size={15} /> Fijada</span>}
        </div>
        <p className="mt-5 whitespace-pre-wrap text-sm leading-6 text-slate-700">{task.description || "Sin descripciÃ³n."}</p>
        <div className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
          <p className={`flex items-center gap-2 font-semibold ${overdue ? "text-red-700" : ""}`}><CalendarClock size={18} /> {task.deadline ? format(new Date(task.deadline), "d MMM yyyy, HH:mm", { locale: es }) : "Sin fecha lÃ­mite"}{overdue && " Â· Vencida"}</p>
        ...
        {!detail ? <p className="py-8 text-center text-sm text-slate-500">Cargando actividad...</p> : tab === "comments" ? (
          <div className="mt-4">
            <div className="max-h-64 space-y-3 overflow-y-auto">
              {detail.comments.length === 0 ? <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">TodavÃ­a no hay comentarios.</p> : detail.comments.map((item) => (
            ...
            {detail.capabilities.canComment && <div className="mt-4 flex gap-2"><textarea value={comment} onChange={(event) => setComment(event.target.value)} rows={2} placeholder="Escribe un comentarioâ€¦" className="min-w-0 flex-1 resize-none rounded-xl border border-slate-300 px-3 py-2.5" /><button disabled={busy || !comment.trim()} onClick={() => void addComment()} className="self-end rounded-xl bg-indigo-600 p-3 text-white disabled:opacity-50" aria-label="Publicar comentario"><Send size={19} /></button></div>}
          </div>
        ) : tab === "history" ? (
          <div className="mt-4 max-h-72 space-y-3 overflow-y-auto">
            {detail.history.length === 0 && detail.requests.length === 0 ? <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No hay cambios registrados.</p> : <>
              {detail.requests.map((request) => <article key={request.id} className="rounded-xl border border-slate-200 p-4 text-sm"><div className="flex flex-wrap justify-between gap-2"><p><strong>{request.requester?.full_name ?? "Colaborador/a"}</strong> solicitÃ³ {statusLabels[request.requested_status].toLowerCase()}.</p><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold">{requestLabels[request.review_status]}</span></div>{request.manager_comment && <p className="mt-2 text-slate-600">Comentario: {request.manager_comment}</p>}<time className="mt-2 block text-xs text-slate-500">{new Date(request.created_at).toLocaleString("es-EC")}</time></article>)}
              {detail.history.map((item) => <article key={item.id} className="rounded-xl border border-slate-200 p-4 text-sm"><p><strong>{item.user?.full_name ?? "Gestor/a"}</strong> cambiÃ³ el estado de {statusLabels[item.old_status].toLowerCase()} a <strong>{statusLabels[item.new_status].toLowerCase()}</strong>.</p><time className="mt-2 block text-xs text-slate-500">{new Date(item.created_at).toLocaleString("es-EC")}</time></article>)}
            </>}
          </div>
        ) : (
          <div className="mt-4">
              ...
              return <article key={file.id} className="rounded-xl border border-slate-200 p-3">
                <div className="flex items-center gap-2">
                  <a href={file.drive_web_url} target="_blank" rel="noreferrer" className="flex min-w-0 flex-1 items-center gap-3">
                    ...
                    <div className="min-w-0"><p className="truncate text-sm font-bold">{file.file_name}</p><p className="text-xs text-slate-500">{formatBytes(file.file_size)} Â· {file.uploader?.full_name ?? "Usuario"}</p></div>
                  </a>
                ...
                {file.review_comment && <p className="mt-2 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-600">Comentario de revisiÃ³n: {file.review_comment}</p>}
...
function formatBytes(value?: number | null) {
  if (!value) return "TamaÃ±o no disponible";
