...
export async function PATCH(request: Request, context: Context) {
  ...
  try {
    ...
    if (folder_id) {
      const { data: folder } = await supabase.from("task_folders").select("id").eq("id", folder_id).eq("company_id", auth.user.companyId!).maybeSingle();
    ...
    if (!data) return NextResponse.json({ error: "Solo puedes organizar tareas creadas por ti" }, { status: 403 });
