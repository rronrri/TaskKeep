...
export async function GET() {
  ...
  const { data, error } = await createAdminClient()
    .from("users")
    .select("id,full_name,email,role,created_at,must_change_password,google_email,google_connected_at,company:companies!users_company_id_fkey(name)")
    .eq("id", auth.user.id)
    .single();
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .update(update)
      .eq("id", auth.user.id)
      .select("id,company_id,full_name,email,role,created_at,must_change_password,google_email,google_connected_at,company:companies!users_company_id_fkey(name)")
      .single();
